...
class FilteredDistillationDataset:
    ...
    def __init__(self, base_distillation_dataset, class_mapping, target_classes, target_soft_labels_info):
        ...
        logger.info(f"FilteredDistillationDataset initialized:")
        logger.info(f"  Base dataset samples: {len(self.base_dataset)}")
        logger.info(f"  Filtered to {self.num_classes} classes")
        logger.info(f"  Class mapping: {self.class_mapping}")
        logger.info(f"  Soft label indices to keep: {self.soft_label_indices}")
    ...
    def __getitem__(self, idx):
        ...
        if original_hard_label not in self.class_mapping:
            ...
            logger.info(f"Warning: Sample {idx} has class {original_hard_label} not in filtered classes")
            logger.info(f"Available class mapping: {self.class_mapping}")
...
class DistillationBirdSoundDataset:
    ...
    def __init__(self, soft_labels_path, **dataset_kwargs):
        ...
        logger.info(f"DistillationDataset initialized:")
        logger.info(f"  Base dataset samples: {len(self.base_dataset)}")
    ...
    def _load_soft_labels(self):
        ...
        logger.info(f"Teacher confidence threshold: {self.soft_labels_metadata.get('confidence_threshold', 'Unknown')}")
        ...
        try:
            ...
            if rel_path in self.soft_labels_data:
                ...
                if np.random.random() < 0.01:  # Print only 1% of missing files
                    logger.info(f"Warning: No soft labels found for {rel_path}")
                    logger.info(f"Using default: zero vector")
            
            return soft_labels
            
        except Exception as e:
            # Fallback based on file path
            logger.info(f"Error getting soft labels for {file_path}: {e}")
            soft_labels = np.zeros(self.num_classes, dtype=np.float32)
            return soft_labels
    ...
    def get_classes(self):
        ...
        if not target_species or len(target_species) != self.num_classes:
            logger.info("Warning: 'target_species' in metadata doesn't match 'num_classes'. Falling back to generic names.")
...
def create_distillation_dataloader(config, soft_labels_path, split='train'):
    ...
    logger.info(f"Creating {split} DataLoader with:")
    logger.info(f"  batch_size: {batch_size}")
    logger.info(f"  num_workers: {num_workers}")
    logger.info(f"  pin_memory: {pin_memory}")
    logger.info(f"  shuffle: {split == 'train'}")
...
def test_distillation_dataset():
    ...
    try:
        ...
        logger.info(f"Hard label: {hard_label}")
        logger.info(f"Soft labels shape: {soft_labels.shape}")
        logger.info(f"Soft labels sum: {soft_labels.sum():.4f}")
        logger.info("Test passed!")
        
    except FileNotFoundError as e:
        logger.info(f"Test skipped (missing files): {e}")
    except Exception as e:
