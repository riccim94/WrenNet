...
class EmptySegmentDataset(Dataset):
    ...
    def __init__(self, bird_data_dir, allowed_bird_classes, no_birds_label,
                 ...
                 seed=42, cache_path=None, refresh_cache=False): # Seed for shuffling the master pool and file list
        """
        ...
        if not EmptySegmentDataset._pool_initialized and not EmptySegmentDataset._scan_in_progress:
            ...
            if self.cache_path and self.cache_path.exists() and not self.refresh_cache:
                try:
                    EmptySegmentDataset._master_empty_segment_pool = self._load_cache()
                    EmptySegmentDataset._pool_initialized = True
                    loaded_from_cache = True
                    print(f"--- Loaded master empty segment pool from {self.cache_path} "
                          f"({len(EmptySegmentDataset._master_empty_segment_pool)} segments). ---")
                except Exception as exc:
                    print(f"WARNING: Failed to load empty segment cache from {self.cache_path}: {exc}. "
            ...
            if not loaded_from_cache:
                ...
                try:
                    ...
                    EmptySegmentDataset._pool_initialized = True
                    print(f"--- Master empty segment pool populated with "
                          f"{len(EmptySegmentDataset._master_empty_segment_pool)} segments. ---")
                    if self.cache_path:
                        try:
                            self._save_cache()
                            print(f"--- Saved empty segment cache to {self.cache_path} ---")
                        except Exception as exc:
                            print(f"WARNING: Failed to save empty segment cache to {self.cache_path}: {exc}")
                finally:
    ...
    def _serialize_pool(self):
        return [
            {"path": path, "start": float(start_time), "end": float(end_time)}
            for path, start_time, end_time in EmptySegmentDataset._master_empty_segment_pool
        ]
    ...
    def _load_cache(self):
        with self.cache_path.open("r", encoding="utf-8") as f:
            data = json.load(f)
        return [(item["path"], float(item["start"]), float(item["end"])) for item in data]
